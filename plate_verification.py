...
class Verificator:
    ...
    def verify_car(self, plate_num):
        ...
        if plate_num in self.car_list:
            ...
            print('car_obj_verified.plate_num')
            print(car_obj_verified.plate_num)
            ...
            try:
                if db_man.get_car_entries(car_obj_verified.plate_num):
                    ...
                    if self.curr_time >= self.cooldown:
                        ...
                        new_entry = Entry(plate = car_obj_verified.plate_num , entry_dtime = curr_time_new_entry, statu='verified') 
            ...
            if(i==10):
                ...
                for i in range (10):
                    if (li[i]<li[i+1]):
                        platex=l[i+1]
                curr_time_new_entry = datetime.now().isoformat(' ', 'seconds')
                new_entry = Entry(plate = plate_num , entry_dtime = curr_time_new_entry, statu='unverified') 
                db.session.add(new_entry) 
                db.session.commit() 
                db_man.get_db_data()
            '''    
            
    def addnotregistred(self, plate_num):
        plate_num = plate_num.upper()
        self.current_plate = plate_num
        ...
        print(self.li)
        while (len(self.li)>1):
            if (len(self.li[i])<len(self.li[i+1])):
                self.li.remove(self.li[i])
            else:
                self.li.remove(self.li[i+1])
            print(self.li)
        print('final')
        self.platex=self.li[0]
        print(self.platex)
        curr_time_new_entry = datetime.now().isoformat(' ', 'seconds')
        new_entry = Entry(plate = self.platex , entry_dtime = curr_time_new_entry, statu='unverified') 
        db.session.add(new_entry) 
        db.session.commit() 
        db_man.get_db_data()
